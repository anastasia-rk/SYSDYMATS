...
\begin{equation}
\underbrace{\mathbf{A}^{\star}}_{M \times K} = \underbrace{\mathbf{U}}_{M \times K}\underbrace{\mathbf{D}}_{K \times K}\underbrace{\mathbf{V}^{\top}}_{K \times K},
\end{equation}
where columns of $\mathbf{U}$ are principal components of $\mathbf{A}^{\star}$, diagonal elements of  $\mathbf{D}$, and where $\mathbf{V}$ is the rotation. All orthonormality assumption are the same as in the general case. The ridge regression \eqref{eq:ridge} then takes form
\begin{equation}
