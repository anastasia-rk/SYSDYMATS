...
ylabel('AAMDL')
%%
tikzName = ['AAMDL_',dataset,'_ny_',num2str(n_y),'_nu_',num2str(n_u),'.tikz'];
cleanfigure;
...
%% Store to table
Iteration = [1:finalTerm]';
temp      = AAMDL_all(1:finalTerm)';
AAMDL     = round(temp,3);
AEERmax  = round(AEER_mm(1:finalTerm,1),5);
Parameters    = round(Theta,2);
Table_all = table(Iteration,Terms,Parameters,AEERmax,AAMDL)
tableName = ['Thetas_',dataset,'_ny_',num2str(n_y),'_nu_',num2str(n_u)];
