...
coeffs_sp = 0:0.1:1;  % for group lasso
iLambda = 0;
...
%             B_gl   = linsolve(R_all,g_gl,struct('UT', true)); 
...
% colormap(my_map)
% plot3(plotCoeffs(row_gl,col_gl),plotAlphas(row_gl,col_gl),min_val_gl,'*b','Linewidth',5);
% set(gca,'XScale','log')
...
colormap(my_map)
plot3(plotCoeffs(row_spl,col_spl),plotAlphas(row_spl,col_spl),min_val_spl,'*b','Linewidth',5);
set(gca,'XScale','log')
xlabel('$\lambda$');ylabel('$\alpha$');zlabel('PRESS');
...
%% group lasso
lambda_gl_opt = Coeffs(row_gl); alpha_gl_opt = coeffs_sp(col_gl); 
g_gl   = group_lasso(M_group, Y_all, lambda_gl_opt, p_group, rho, 1+alpha_gl_opt); 
B_gl   = linsolve(R_group,g_gl,struct('UT', true)); 
Betas_gl_opt  = reshape(B_gl,[finalTerm,L])
%% sparse group lasso
lambda_spl_opt = Coeffs(row_spl); 
alpha_spl_opt = coeffs_sp(col_spl); 
...
%% sparse group non-orthogonal
% B_nonort_spl  = SPLAsso(Y_all, M_all, p_sparesgroup, (1-alpha_spl_opt)*lambda_spl_opt, alpha_spl_opt*lambda_spl_opt); 
% Betas_nonortspl_opt = reshape(B_spl,[finalTerm,L])
...
Tab = table(Step,Terms);
for iBeta=1:L
    Parameters = round(Betas_spl_opt(:,iBeta),2);
    varName = ['$\beta_{',num2str(iBeta-1),'}$'];
    Tab = addvars(Tab,Parameters,'NewVariableNames',varName);
end
...
Tab = table(Step,Terms);
for iBeta=1:L
    Parameters = round(Betas_nonreg_opt(:,iBeta),2);
    varName = ['$\beta_{',num2str(iBeta-1),'}$'];
    Tab = addvars(Tab,Parameters,'NewVariableNames',varName);
end
...
Tab = table(Step,Terms);
for iBeta=1:L
    Parameters = round(Betas_gl_opt(:,iBeta),2);
    varName = ['$\beta_{',num2str(iBeta-1),'}$'];
    Tab = addvars(Tab,Parameters,'NewVariableNames',varName);
end
tableName = [folderName,'/Betas_group'];
table2latex(Tab,tableName);
clear Tab
Tab = table(Step,Terms);
for iBeta=1:L
...
matlab2tikz(tikzName, 'showInfo', false,'parseStrings',false,'standalone', ...
            false, 'height', '4cm', 'width','15cm','checkForUpdates',false);
%% Validate group lasso
Theta_test  = Betas_gl_opt*A_valid';
iTheta      = 0;
figure('Name','Outputs','NumberTitle','off','visible',visFlag);
index_test  = 1000:3000;
for iFile = testFiles
...
tikzName = [folderName,'/','Group_lasso_validation.tikz'];
cleanfigure;
matlab2tikz(tikzName, 'showInfo', false,'parseStrings',false,'standalone', ...
            false, 'height', '4cm', 'width','15cm','checkForUpdates',false);
%% Save all workspace
