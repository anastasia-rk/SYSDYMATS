...
% Analysis for scaled data matrix
R_ss = A_s'*A_s
eig_r = eig(R_ss);
kappa = cond(A_s);
thr = 100;
...
for iTheta = 1:finalTerm
        ...
        vect = beta{iTheta};
        semilogx(coeffs(ind(iTheta))*ones(2),[min(min(vect)) max(max(vect))],'-k','Linewidth',2); hold on;
        ...
        vect = beta_raw{iTheta};
        semilogx(coeffs(ind_raw(iTheta))*ones(2),[min(min(vect)) max(max(vect))],'-k','Linewidth',2); hold on;
        title('Tikhonov raw')
        xlabel('$\gamma$');
